...
// TODO do we need discriminator
//// Ripped from built_value
///
...
class MajorGraphQLJsonPlugin implements SerializerPlugin {
  ...
  MajorGraphQLJsonPlugin({this.discriminator = r'$', this.valueKey = ''});
  ...
  Object beforeDeserialize(Object object, FullType specifiedType) {
    if (object is Map) {
      if (specifiedType.root != JsonObject) {
        if (specifiedType.isUnspecified) {
          return _toListUsingDiscriminator(object as Map<String, Object>);
        } else {
          return _toList(
  ...
  Map _toMap(List list, bool needsEncodedKeys) {
    ...
    for (var i = 0; i != list.length ~/ 2; ++i) {
      final dynamic key = list[i * 2];
      final dynamic value = list[i * 2 + 1];
  ...
  Map _toMapWithDiscriminator(List list) {
    ...
    for (var i = 0; i != (list.length - 1) ~/ 2; ++i) {
      ...
      final dynamic value = list[i * 2 + 2];
      result[key] = value;
    }
    return result;
  }
  ...
  List _toList(Map<String, Object> map, bool hasEncodedKeys) {
    var result = List<Object>(map.length * 2);
    var i = 0;
    map.forEach((key, value) {
      // Drop null values, they are represented by missing keys.
      if (value == null) return;

      result[i] = hasEncodedKeys ? _decodeKey(key) : key;
      result[i + 1] = value;
      i += 2;
    });
    return result;
  ...
  List<Object> _toListUsingDiscriminator(Map<String, Object> map) {
    ...
    var result = List<Object>(map.length * 2 - 1);
    result[0] = type;
    ...
    map.forEach((key, value) {
      ...
      result[i] = needToDecodeKeys ? _decodeKey(key) : key;
      result[i + 1] = value;
      i += 2;
    });
    return result;
